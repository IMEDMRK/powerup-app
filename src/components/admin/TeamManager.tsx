...
export default function TeamManager({ initialUsers, landingPages = [] }: { initialUsers: any[], landingPages?: any[] }) {
  ...
  const resetForm = () => {
    setFormData({
      ...
      username: "",
      password: "",
      permissions: { canViewStats: false, canManageDelivery: false, canManagePages: false, canManageSettings: false, canManageAllPages: true, allowedPages: [] },
      commissionPerDelivered: 0,
    });
    setEditingId(null);
  };
          ...
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={saveUser} className="p-6">
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-100">
                  <label className="block text-sm font-bold text-gray-700 mb-3">صلاحيات إضافية (اختياري)</label>
                  <p className="text-xs text-gray-500 mb-4 leading-relaxed">بشكل افتراضي، يرى الموظف الطلبيات فقط (بدون أسعار وإجمالي الأرباح) ولا يمكنه تغيير الإعدادات. يمكنك إعطائه صلاحيات إضافية هنا:</p>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200 transition-colors">
                      ...
                      <span className="text-sm font-medium text-gray-700">رؤية الإحصائيات والأرباح المالية</span>
                    </label>
                    <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200 transition-colors">
                      ...
                      <span className="text-sm font-medium text-gray-700">توليد بوليصات التوصيل (إرسال لشركة التوصيل)</span>
                    </label>
                    <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200 transition-colors">
                      <input type="checkbox" checked={formData.permissions.canManagePages} onChange={() => togglePermission("canManagePages")} className="w-5 h-5 rounded text-primary focus:ring-primary" />
                      <span className="text-sm font-medium text-gray-700">إنشاء وتعديل صفحات الهبوط</span>
                    </label>
                    <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer border border-transparent hover:border-gray-200 transition-colors">
                      ...
                      <span className="text-sm font-medium text-gray-700">تعديل الإعدادات والبيكسل والتوصيل</span>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 flex gap-3">
                ...
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl transition-all">
                  إلغاء
                </button>
              </div>
            </form>
