...
function VideoCard({ video, pColor }: { video: Video; pColor: string }) {
  ...
  return (
    <div 
      className="bg-white rounded-3xl overflow-hidden shadow-2xl border-[6px] hover:shadow-xl transition-shadow p-2 sm:p-3"
...
export default function VideoSection({
  ...
  const pColor = primaryColor || "#f97316";
    ...
    <section className="py-10 sm:py-16 bg-gray-50" id="videos">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className={`grid gap-6 ${
          ...
          {videos.map(v => <VideoCard key={v.id} video={v} pColor={pColor} />)}
        </div>
      </div>
    </section>
  );
