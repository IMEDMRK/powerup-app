...
export default async function TeamPerformancePage({ searchParams }: { searchParams: { month?: string } }) {
  ...
  const performanceData = users.map(u => {
    ...
    // Calculate commissions including upsells
    const commission = (u.ordersConfirmed || []).reduce((acc: number, o: any) => {
      if (o.status !== "مستلمة") return acc;
      
      let orderCommission = u.commissionPerDelivered || 0;
      
      // Upsell logic: if original quantity was 1, and final quantity is > 1
      if (o.initialQuantity === 1 && o.quantity > 1) {
        const extraBoxes = o.quantity - 1;
        const extraCommission = extraBoxes * (u.upsellCommission || 0);
        orderCommission += extraCommission;
      }
