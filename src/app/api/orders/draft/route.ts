...
export async function POST(request: Request) {
  try {
    ...
    // Check if there is already a COMPLETED order for this phone in the last 15 minutes to prevent race conditions
    const existingCompleted = await prisma.order.findFirst({
      where: {
        phone: safePhone,
        status: { not: "غير مكتملة" },
        createdAt: { gte: new Date(Date.now() - 15 * 60 * 1000) }
      }
    });

    if (existingCompleted) {
      return NextResponse.json({ success: true, orderId: existingCompleted.id }, { status: 200 });
    }
