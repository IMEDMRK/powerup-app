...
function download(sha) {
  https.get(`https://raw.githubusercontent.com/IMEDMRK/powerup-app/${sha}/.env.production`, (res) => {
    if (res.statusCode === 200) {
      let data = '';
      res.on('data', d => data += d);
      res.on('end', () => console.log(`--- .env.production at ${sha} ---\n${data}`));
    } else {
      console.log(`No .env.production at ${sha} (Status: ${res.statusCode})`);
    }
  });
}
